...
MIRROR_REPORT_PATH = ROOT / "reports" / "RESULT.md"
TOP_K = 5
...
|---:|---|---|---|---|
| 1 | Replace hashing baseline with a Vietnamese/multilingual sentence-transformer | Dense misses paraphrases in low-recall cases | Higher semantic recall | Re-run the same {len(dense_rows)} cases and compare recall |
...
"""
    for path in (REPORT_PATH, MIRROR_REPORT_PATH):
...
def main() -> None:
    ...
    print(f"Evaluated {len(dataset)} cases; reports: {REPORT_PATH}, {MIRROR_REPORT_PATH}")
